...
def valid_coin(obj: Dict[str, Any]) -> bool:
...
def send_alerts(bot: Bot, coins):
    # tenta carregar TELEGRAM_CHAT_ID do arquivo se houver
    chat_id_env = TELEGRAM_CHAT_ID
    try:
        with open(CHAT_ID_FILE) as f:
            chat_id_file = f.read().strip()
    except Exception:
        chat_id_file = ""

    dest = chat_id_file or chat_id_env
    if not dest:
    ...
    if not coins:
        bot.send_message(
            chat_id=dest,
            text=(
                "🔎 *Radar LowCap*: Nenhum ativo bateu os filtros neste ciclo.\n\n"
...
async def cmd_radar(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    await context.bot.send_message(
        chat_id=chat_id,
        text=(
            "🔎 *Radar LowCap*: buscando candidatos...\n"
            f"🎯 Filtros atuais — MCAP ${MCAP_MIN:,}–${MCAP_MAX:,}; Vol≥${int(MIN_VOL_24H):,}; "
            f"7d≥{MIN_CHANGE_7D}% ; 24h≥{MIN_CHANGE_24H}%"
        ),
        parse_mode="Markdown",
        disable_web_page_preview=True,
    ...
    try:
        ...
        return
    if not coins:
        await context.bot.send_message(chat_id=chat_id, text="Sem resultados neste momento.")
        return
    for c in coins:
        await context.bot.send_message(
            chat_id=chat_id,
...
async def cmd_setchat(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    try:
        with open(CHAT_ID_FILE, "w") as f:
            f.write(str(chat_id))
        await context.bot.send_message(chat_id=chat_id, text=f"✅ Chat {chat_id} salvo como destino padrão.")
    except Exception as e:
...
if __name__ == "__main__":
    ...
    parser.add_argument("--loop", action="store_true", help="alertas automáticos para TELEGRAM_CHAT_ID")
    parser.add_argument("--once", action="store_true", help="executa apenas uma vez")
    parser.add_argument("--poll", action="store_true", help="modo polling com comandos /radar /config /setchat")
